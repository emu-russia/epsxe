...
char __cdecl sub_4314A0(_BYTE *a1, _BYTE *a2)
{
  char result; // al
...
char __cdecl sub_431900(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3, unsigned __int8 a4, BYTE *a5)
{
...
char __cdecl sub_431A20(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3, unsigned __int8 a4, BYTE *a5)
{
  char v6; // [esp+0h] [ebp-Ch] BYREF
  __int16 v7; // [esp+1h] [ebp-Bh]
  char v8; // [esp+3h] [ebp-9h]
...
char __cdecl sub_431AB0(int a1)
{
...
char __cdecl sub_431B00(BYTE *a1)
{
...
char __cdecl W9x_sub_433930(unsigned int a1, unsigned __int8 a2, unsigned __int8 a3, int a4)
{
  char v4; // bl
  char v5; // cl
  char result; // al
...
|--------------------------------|---------------------------------------------|----------------------------------------------------------------------------|
| `byte_504CA2`                  | `W9x_first_track`                           | Номер первого трека                                                        |
| `byte_504CA3`                  | `W9x_last_track`                            | Номер последнего трека                                                     |
| `dword_4FD9BC`                 | `W9x_global_aspi_state`                     | Многоцелевая: LOBYTE – кол-во треков, BYTE1 – число адаптеров, BYTE2 – макс. таргетов, HIBYTE – кол-во найденных устройств |
| `dword_504C8C`                 | `W9x_read_mode`                             | Режим чтения (1–10), определяет формат субканалов                         |
| `dword_504C88`                 | `W9x_sector_size`                           | Размер сектора (2352 или 2368)                                            |
| `dword_504C84`                 | `W9x_subcache_buffer`                       | Указатель на буфер кэша субканалов (для .M3S)                             |
| `byte_4FD9C8`                  | `W9x_subchannel_enabled`                    | Флаг: включена ли поддержка субканалов (1)                                |
| `Stream`                       | `W9x_subchannel_file`                       | Указатель FILE* на открытый .SUB файл                                    |
| `dword_4FFF80`                 | `W9x_last_lba_read`                         | Последний прочитанный LBA (для кэширования)                               |
| `byte_4FFF84`                  | `W9x_read_buffer`                           | Буфер для чтения одного сектора (обычно 2352 байта)                       |
| `dword_5053E0`                 | `W9x_sub_info0`                             | Субканальные данные сектора (часть 1)                                     |
| `dword_5053E4`                 | `W9x_sub_info1`                             | Часть 2                                                                   |
| `dword_5053E8`                 | `W9x_sub_info2`                             | Часть 3                                                                   |
| `dword_5053EC`                 | `W9x_sub_info3`                             | Часть 4                                                                   |
| `dword_504C04`                 | `W9x_subchannel_raw`                        | Массив для хранения сырых субканальных данных (8 секторов)                |
| `dword_4FD9C0`                 | `W9x_cdda_playing`                          | Флаг: идёт ли проигрывание CD‑аудио                                       |
| `dword_4FD9C4`                 | `W9x_cdda_paused`                           | Флаг: поставлено ли на паузу (требуется возобновление)                    |
| `dword_456FB4`                 | `W9x_cdda_start_lba`                        | Начальный LBA для проигрывания                                            |
| `dword_456FB8`                 | `W9x_cdda_end_lba`                          | Конечный LBA для проигрывания                                             |
| `word_456FBC`                  | `W9x_cdda_current_frame`                    | Текущий фрейм (внутри проигрывания)                                       |
| `dword_504FE0` …               | `W9x_device_info[].field1`                  | Массив структур информации об устройствах (смещение = HIBYTE(global)<<6) |
| `byte_504CA6`                  | `W9x_track_info[]`                          | Массив информации о треках (по 8 байт на трек)                            |
| `byte_504CA0`                  | `W9x_toc_buffer`                            | Буфер для TOC (Read TOC)                                                  |
| `nocd`                         | `W9x_no_cd_audio`                           | Флаг: отсутствие аудио-треков (устанавливается в 2)                      |
| `cd_savefake_flag`             | `W9x_fake_cd_mode`                          | Режим эмуляции CD (0 – чтение, 1 – запись, -1 – отключено)               |
| `loaded_file_type`             | `W9x_image_type`                            | Тип загруженного образа (3 – ISO, и т.п.)                                 |
| `auto_ppf_load`                | `W9x_auto_ppf`                              | Флаг автоматического применения PPF-патчей                               |
| `use_subchannel`               | `W9x_use_subchannel`                        | Флаг использования субканалов                                             |
| `SubchannelW9xCaching`         | `W9x_subcache_enabled`                      | Включено ли кэширование субканалов                                        |
| `SubchannelW9xCachingLG`       | `W9x_subcache_lg`                           | Флаг для специфического кэширования (LG)                                 |
| `SubchannelW9xCdromEnabled`    | `W9x_cdrom_subchannel_enabled`              | Разрешено ли чтение субканалов с реального CD-ROM                        |
| `hCdrModule`                   | `W9x_aspi_module`                           | Хэндл загруженной WNASPI32.DLL                                           |
| `SendASPI32Command_cb`         | `W9x_send_aspi_cmd`                         | Указатель на функцию SendASPI32Command                                   |
| `GetASPI32SupportInfo`         | `W9x_get_aspi_info`                         | Указатель на функцию GetASPI32SupportInfo                                |
| `byte_8B3D80`                  | `W9x_volume_label`                          | Имя файла образа (или метка тома)                                        |
| `dword_455945`                 | `W9x_subchannel_flag`                       | Вспомогательный флаг для субканалов                                      |
