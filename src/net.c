...
int net_load_plugin()
{
  ...
  NETopen = (int (__stdcall *)(uint32_t))GetProcAddress(hNetModule, "NETopen");
  if ( !NETopen )
  ...
  NETresume = (int (__stdcall *)(uint32_t))GetProcAddress(hNetModule, "NETresume");
  if ( !NETresume )
    ui_error(" * GetProcAddress error NETresume\n");
  NETpadState = (int (__stdcall *)(uint32_t, uint32_t))GetProcAddress(hNetModule, "NETpadState");
  if ( !NETpadState )
    ui_error(" * GetProcAddress error NETpadState\n");
  NETcompareData = (int (__stdcall *)(uint32_t, uint32_t))GetProcAddress(hNetModule, "NETcompareData");
  if ( !NETcompareData )
  ...
  NETtransferData = (int (__stdcall *)(uint32_t, uint32_t, uint32_t))GetProcAddress(hNetModule, "NETtransferData");
  if ( !NETtransferData )
...
char net_netplay_handler()
{
  ...
  if ( network_enabled )
  {
    ...
    if ( (uint8_t)NETcompareData(default_filename, 12) )
    ...
    if ( (uint8_t)NETcompareData(netplay_start_options, 1) )
      fatal_error_with_message_box(" * NETPLAY: Start Mode option is different in every site. \n");
    if ( (uint8_t)NETcompareData(&netplay_sound_enabled, 1) )
    ...
    controller_port_modes[0] = (uint8_t)netplay_config_packet;
    forcepad = (uint8_t)netplay_config_options;
    ...
    mdectiming = (uint8_t)netplay_config_mdectiming;
...
int net_fill_input()
{
  ...
  if ( network_enabled )
  {
    ...
    {
      controller_port_modes[0] = (uint8_t)netplay_pad1_state;
      ...
      mouse_buttons_state = (uint8_t)netplay_mouse_state;
      joystick_button_state4[0] = netplay_joy4_state;
      controller_port_modes[1] = (uint8_t)netplay_pad2_state;
