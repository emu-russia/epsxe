#include "pch.h"

/* static prototypes for internal functions */
static unsigned int spucore_generate_samples();

static int spucore_init_dsound()
{
  int v0;
  int v1;
  int16_t v3;
  int16_t v4;
  int v5;
  int v6;
  uint16_t v7;
  int16_t v8;
  int16_t v9;

  spu_sample_rate_mode = audio_sample_rate_mode;
  spu_stereo_flag = audio_stereo_flag;
  if ( DirectSoundCreate(nullptr, (LPDIRECTSOUND *)&ppDS, nullptr) )
    return 0;
  ((LPDIRECTSOUND)ppDS)->lpVtbl->SetCooperativeLevel((LPDIRECTSOUND)ppDS, hOutputWnd, 1);
  v3 = 1;
  switch ( audio_sample_rate_mode )
  {
    case 0:
      v0 = 2048;
      v1 = 8000;
      audio_buffer_size_bytes = 2048;
      break;
    case 1:
      v0 = 2048;
      audio_buffer_size_bytes = 2048;
      goto LABEL_11;
    case 2:
      v0 = 4096;
      v1 = 22050;
      audio_buffer_size_bytes = 4096;
      break;
    case 3:
      v0 = 0x2000;
      v1 = 44100;
      audio_buffer_size_bytes = 0x2000;
      break;
    case 4:
      v0 = 4096;
      v1 = 16000;
      audio_buffer_size_bytes = 4096;
      break;
    case 5:
      v0 = 0x2000;
      v1 = 32000;
      audio_buffer_size_bytes = 0x2000;
      break;
    case 6:
      v0 = 0x2000;
      v1 = 48000;
      audio_buffer_size_bytes = 0x2000;
      break;
    default:
      v0 = audio_buffer_size_bytes;
LABEL_11:
      v1 = 11025;
      break;
  }
  v5 = v1;
  if ( audio_stereo_flag == 1 )
  {
    v0 *= 2;
    v4 = 2;
    v7 = 4;
    audio_buffer_size_bytes = v0;
  }
  else
  {
    v4 = 1;
    v7 = 2;
  }
  v8 = 16;
  v6 = v1 * v7;
  memset(&spu_dsound_desc_size, 0, 0x24u);
  spu_dsound_desc_buffer_bytes = v0;
  v9 = 0;
  spu_dsound_desc_size = 36;
  spu_dsound_desc_flags = 16392;
  *(uint32_t *)spu_dsound_wave_format_ptr = &v3;
  if ( ((LPDIRECTSOUND)ppDS)->lpVtbl->CreateSoundBuffer((LPDIRECTSOUND)ppDS, (LPCDSBUFFERDESC)&spu_dsound_desc_size, (LPDIRECTSOUNDBUFFER *)&pDSBuffer, nullptr)
    || ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->Play((LPDIRECTSOUNDBUFFER)pDSBuffer, 0, 0, 1) )
  {
    return 0;
  }
  audio_buffer_delay = 0;
  return 1;
}

static int spucore_update_dsound()
{
  int result;
  DWORD v1;
  int v2;
  int v3;
  char *v4;
  int *v5;
  int v6;
  char *v7;
  char v8;
  unsigned int v9;
  char *v10;

  if ( spu_irq_pending_flag == 1 )
  {
    result = ++spu_irq_counter;
    spu_irq_pending_flag = (unsigned int)spu_irq_counter <= 0x40;
  }
  else
  {
    spu_irq_counter = 0;
    ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->GetCurrentPosition((LPDIRECTSOUNDBUFFER)pDSBuffer, (LPDWORD)&spu_dsound_play_cursor, (LPDWORD)&spu_dsound_write_cursor);
    v1 = spu_dsound_buffer_pos;
    result = spu_dsound_play_cursor;
    if ( spu_dsound_buffer_pos > (unsigned int)spu_dsound_play_cursor )
    {
      v2 = spu_dsound_play_cursor + audio_buffer_size_bytes - spu_dsound_buffer_pos;
    }
    else
    {
      result = spu_dsound_play_cursor - spu_dsound_buffer_pos;
      v2 = spu_dsound_play_cursor - spu_dsound_buffer_pos;
    }
    for ( ; v2 >= 256; v2 -= 256 )
    {
      if ( spu_irq_pending_flag == 1 )
        break;
      v3 = spu_mute_flag;
      if ( !spu_mute_flag )
      {
        spucore_generate_samples();
        v1 = spu_dsound_buffer_pos;
        v3 = spu_mute_flag;
      }
      v4 = temp_samples;
      v5 = &spucore_output_buffer;
      do
      {
        *(uint16_t *)v4 = *(uint16_t *)v5;
        v6 = *v5;
        if ( *v5 > 0x7FFF )
          *(uint16_t *)v4 = 0x7FFF;
        if ( v6 < -32767 )
          *(uint16_t *)v4 = -32767;
        if ( v3 )
          *(uint16_t *)v4 = 0;
        ++v5;
        v4 += 2;
      }
      while ( (int)v5 < (int)&spu_output_samples[508] );
      result = ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->Lock(
                 (LPDIRECTSOUNDBUFFER)pDSBuffer,
                 v1,
                 256,
                 (LPVOID *)&lock_ptr1,
                 (LPDWORD)&lock_size1,
                 (LPVOID *)&lock_ptr2,
                 (LPDWORD)&lock_size2,
                 0);
      if ( result )
        break;
      qmemcpy((void *)lock_ptr1, temp_samples, lock_size1);
      v7 = (char *)lock_ptr2;
      if ( lock_ptr2 )
      {
        v8 = lock_size2;
        v9 = (unsigned int)lock_size2 >> 2;
        v10 = (char *)(2 * lock_size1 + 4602128);
        qmemcpy((void *)lock_ptr2, v10, 4 * ((unsigned int)lock_size2 >> 2));
        qmemcpy(&v7[4 * v9], &v10[4 * v9], v8 & 3);
        v7 = (char *)lock_ptr2;
      }
      result = ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->Unlock((LPDIRECTSOUNDBUFFER)pDSBuffer, (LPVOID)lock_ptr1, lock_size1, v7, lock_size2);
      if ( result )
        break;
      result = audio_buffer_size_bytes;
      v1 = spu_dsound_buffer_pos + 256;
      spu_dsound_buffer_pos += 256;
      if ( spu_dsound_buffer_pos >= (unsigned int)audio_buffer_size_bytes )
      {
        v1 = 0;
        spu_dsound_buffer_pos = 0;
      }
    }
  }
  return result;
}

static int spucore_init_gauss_table()
{
  int v0;
  int result;
  char *v2;
  int v3;
  int v4;
  char v5;

  v0 = 0;
  result = 0;
  v2 = spu_gauss_table;
  v3 = 256;
  do
  {
    v4 = 128;
    do
    {
      v5 = spu_gauss_kernel[(uint8_t)v0++] + spu_gauss_kernel[(uint8_t)result--];
      *v2++ = v5;
      --v4;
    }
    while ( v4 );
    --result;
    --v3;
  }
  while ( v3 );
  return result;
}

static int spucore_voice_key_on(int a1)
{
  int result;
  uint32_t adsr_sustain_dir;

  result = a1;
  adsr_sustain_dir = spu_voice_param[a1].adsr_sustain_dir;
  spu_voice_param[result].current_block_addr = 1;
  spu_voice_param[result].sample_history[1] = 0;
  spu_voice_param[result].sample_history[2] = adsr_sustain_dir;
  spu_voice_param[result].adpcm_s0 = 0;
  spu_voice_param[result].unknown3E = 0;
  return result * 296;
}

static int spucore_voice_key_off(int a1)
{
  int result;

  result = 296 * a1;
  if ( spu_voice_param[a1].current_block_addr )
  {
    spu_voice_param[a1].current_block_addr = 4;
  }
  else
  {
    spu_voice_param[a1].unknown17 = 0;
    spu_voice_param[a1].sample_history[1] = 0;
  }
  return result;
}

static uint8_t spucore_decode_adpcm_block(int a1)
{
  uint16_t v1;
  int32_t v2;
  char v3;
  int v4;
  uint32_t pitch_multiplier;
  uint8_t v6;
  int v7;
  uint8_t result;
  int16_t v9;
  int v10;
  int v11;
  int32_t v12;
  int v13;
  int v14;
  uint8_t v15;
  int v16;
  char v17;
  int32_t v18;

  v1 = spu_ctrl_status;
  v2 = spu_voice_param[a1].sample_history[2];
  v3 = BYTE1(spu_ram[2 * v2]);
  v18 = v2;
  if ( (uint16_t)spu_ctrl_status == v2 && (spu_cnt & 0x40) != 0 && !spu_irq_pending_flag )
  {
    spu_irq_pending_flag = 1;
    irq_spu_registered_callback();
    v1 = spu_ctrl_status;
  }
  if ( v1 == spu_voice_param[a1].sample_history[2] + 1 && (spu_cnt & 0x40) != 0 && !spu_irq_pending_flag )
  {
    spu_irq_pending_flag = 1;
    irq_spu_registered_callback();
    v1 = spu_ctrl_status;
  }
  v4 = spu_voice_param[a1].sample_history[2] + 2;
  spu_voice_param[a1].sample_history[2] = v4;
  spu_voice_param[a1].unknown3E = 1;
  if ( (v3 & 1) == 0 || (pitch_multiplier = spu_voice_param[a1].pitch_multiplier) == 0 || v1 != pitch_multiplier )
  {
    switch ( v3 )
    {
      case 1:
      case 7:
        spu_voice_param[a1].unknown3E = 2;
        result = 0;
        goto LABEL_26;
      case 3:
        spu_voice_param[a1].sample_history[2] = spu_voice_param[a1].pitch_multiplier;
        goto LABEL_13;
      case 6:
        spu_voice_param[a1].pitch_multiplier = v4 - 2;
        goto LABEL_13;
      default:
        goto LABEL_13;
    }
  }
  spu_voice_param[a1].sample_history[2] = pitch_multiplier;
LABEL_13:
  v6 = spu_ram[2 * v2];
  v7 = 2 * (v6 >> 4);
  v16 = spu_adpcm_coeffs_a[v7];
  v15 = 0;
  v14 = spu_adpcm_coeffs_b[v7];
  v17 = v6 & 0xF;
  while ( 1 )
  {
    v9 = *((uint8_t *)&spu_ram[2 * v2] + v15 + 2);
    v10 = ((int16_t)(v9 << 12) >> v17)
        + (v16 * spu_voice_param[a1].sample_history[3] + v14 * spu_voice_param[a1].sample_history[4]) / 64;
    if ( v10 > 0x7FFF )
      break;
    if ( v10 < -32768 )
      break;
    spu_voice_param[a1].sample_history[4] = spu_voice_param[a1].sample_history[3];
    spu_voice_param[a1].sample_history[3] = v10;
    v11 = 2 * (v15 + 37 * a1);
    spu_voice_param[0].sample_history[v11 + 5] = v10;
    v12 = spu_voice_param[a1].sample_history[3];
    v13 = ((int16_t)((v9 & 0xFFF0) << 8) >> v17) + (v16 * v12 + v14 * spu_voice_param[a1].sample_history[4]) / 64;
    if ( v13 > 0x7FFF || v13 < -32768 )
      break;
    spu_voice_param[a1].sample_history[3] = v13;
    spu_voice_param[a1].sample_history[4] = v12;
    spu_voice_param[0].sample_history[v11 + 6] = v13;
    result = ++v15;
    if ( v15 >= 0xEu )
      return result;
    v2 = v18;
  }
  result = 0;
  ++spu_adpcm_error_count;
LABEL_26:
  memset(&spu_voice_param[a1].sample_history[5], 0, 0x70u);
  return result;
}

static unsigned int spucore_generate_samples()
{
  int v0;
  int16_t v1;
  int v2;
  double v3;
  char *v4;
  int v5;
  int v6;
  int v7;
  int v8;
  int16_t *i;
  unsigned int result;
  unsigned int v11;
  unsigned int v12;
  int v13;
  int v14;
  int v15;
  int v16;
  bool v17;
  int v18;
  int v19;
  int v20;
  int v21;
  int v22;
  char *v23;
  unsigned int v24;
  int v25;
  int32_t v26;
  int v27;
  int v28;
  int v29;
  int v30;
  float v31;
  int v32;

  v0 = spu_xa_samples_left;
  v1 = HIWORD(spu_xa_playback_pos);
  v2 = SHIWORD(spu_xa_playback_pos);
  v31 = (double)spu_xa_playback_rate * 0.00002267573696145125;
  if ( spu_xa_samples_left | SHIWORD(spu_xa_playback_pos) )
  {
    v3 = flt_4F75AC;
    v4 = spu_output_samples;
    if ( spu_xa_stereo_flag )
    {
      do
      {
        LOWORD(spu_xa_playback_pos) = (int64_t)v3;
        if ( (uint16_t)spu_xa_playback_pos >= v2 && v1 > 0 )
        {
          if ( !v0 )
            spu_xa_playback_rate -= 250;
          if ( 2 * v0 > 0 )
            qmemcpy(spu_xa_play_pcm, spu_xa_decoded_pcm, 4 * ((unsigned int)(2 * v0) >> 1));
          v3 = 0.0;
          v1 = v0;
          v0 = 0;
          spu_xa_samples_left = 0;
          LOWORD(spu_xa_playback_pos) = 0;
        }
        v2 = v1;
        if ( (uint16_t)spu_xa_playback_pos >= v1 )
        {
          *((uint32_t *)v4 - 1) = 0;
          *(uint32_t *)v4 = 0;
        }
        else
        {
          v3 = v3 + v31;
          v5 = 2 * (spu_xa_playback_pos & 0x3FFE);
          v6 = spu_xa_play_pcm[v5];
          v7 = spu_xa_play_data[v5];
          *((uint32_t *)v4 - 1) = v6;
          *(uint32_t *)v4 = v7;
        }
        v4 += 8;
      }
      while ( (int)v4 < (int)spu_output_samples_end );
      flt_4F75AC = v3;
      HIWORD(spu_xa_playback_pos) = v1;
    }
    else
    {
      do
      {
        LOWORD(spu_xa_playback_pos) = (int64_t)v3;
        if ( (uint16_t)spu_xa_playback_pos >= 2 * v2 && v1 > 0 )
        {
          if ( !v0 )
            spu_xa_playback_rate -= 250;
          if ( 2 * v0 > 0 )
            qmemcpy(spu_xa_play_pcm, spu_xa_decoded_pcm, 4 * ((unsigned int)(2 * v0) >> 1));
          v3 = 0.0;
          v1 = v0;
          v0 = 0;
          spu_xa_samples_left = 0;
          LOWORD(spu_xa_playback_pos) = 0;
        }
        v2 = v1;
        if ( (uint16_t)spu_xa_playback_pos >= 2 * v1 )
        {
          *((uint32_t *)v4 - 1) = 0;
          *(uint32_t *)v4 = 0;
        }
        else
        {
          v3 = v3 + v31;
          v8 = spu_xa_play_pcm[spu_xa_playback_pos & 0x3FFF];
          *((uint32_t *)v4 - 1) = v8;
          *(uint32_t *)v4 = v8;
        }
        v4 += 8;
      }
      while ( (int)v4 < (int)spu_output_samples_end );
      flt_4F75AC = v3;
      HIWORD(spu_xa_playback_pos) = v1;
    }
  }
  else
  {
    memset(&spucore_output_buffer, 0, 0x200u);
  }
  v30 = 0;
  v32 = 0;
  for ( i = (uint16_t *)(spu_voice_param + 94); (int)i < 4616542; i += 148 )
  {
    result = *(uint32_t *)(i + 3);
    if ( result && (((unsigned int)spucore_pitchmod_enable >> (v30 + 1)) & 1) == 0 )
    {
      v11 = ((unsigned int)(uint16_t)spucore_mainvol_left * *(uint32_t *)(i - 47)) >> 20;
      v12 = ((unsigned int)(uint16_t)spucore_mainvol_right * *(uint32_t *)(i - 45)) >> 20;
      switch ( result )
      {
        case 1u:
          v13 = (*(uint32_t *)(i + 91) << 6) + *(uint32_t *)(i - 1);
          *(uint32_t *)(i - 1) = v13;
          if ( v13 >= 0xFFFFFF )
          {
            *(uint32_t *)(i - 1) = 0xFFFFFF;
            *(uint32_t *)(i + 3) = 2;
          }
          break;
        case 2u:
          v14 = (*(uint32_t *)(i + 93) << 6) + *(uint32_t *)(i - 1);
          v15 = *(uint32_t *)(i + 95);
          *(uint32_t *)(i - 1) = v14;
          if ( v14 <= v15 )
          {
            *(uint32_t *)(i - 1) = v15;
            *(uint32_t *)(i + 3) = 3;
          }
          break;
        case 3u:
          v16 = *(uint32_t *)(i - 1) + (*(uint32_t *)(i + 97) << 6);
          v17 = *(uint32_t *)(i - 9) == 1;
          *(uint32_t *)(i - 1) = v16;
          if ( v17 )
          {
            if ( v16 <= 0 )
            {
              *(uint32_t *)(i - 1) = 0;
              *(uint32_t *)(i + 3) = 0;
            }
          }
          else if ( v16 >= 0xFFFFFF )
          {
            *(uint32_t *)(i + 3) = 5;
            goto LABEL_41;
          }
          break;
        case 4u:
          v20 = (*(uint32_t *)(i + 99) << 6) + *(uint32_t *)(i - 1);
          *(uint32_t *)(i - 1) = v20;
          if ( v20 <= 0 )
          {
            *(uint32_t *)(i - 1) = 0;
            *(uint32_t *)(i + 3) = 0;
          }
          break;
        case 5u:
LABEL_41:
          *(uint32_t *)(i - 1) = 0xFFFFFF;
          break;
        default:
          break;
      }
      v18 = *i;
      if ( *(uint32_t *)(i - 39) )
        v19 = 0xFFFF - v11 * v18;
      else
        v19 = v11 * v18;
      v21 = v12 * v18;
      if ( *(uint32_t *)(i - 37) )
        v22 = 0xFFFF - v21;
      else
        v22 = v21;
      v23 = spu_output_samples;
      do
      {
        if ( !*(uint32_t *)(i + 77) )
        {
          do
          {
            v24 = *(uint32_t *)(i + 7);
            if ( v24 > 0x1BFFFF )
            {
              do
                v24 -= 1835008;
              while ( v24 > 0x1BFFFF );
              *(uint32_t *)(i + 77) = 0;
              *(uint32_t *)(i + 7) = v24;
            }
            spucore_decode_adpcm_block(v30);
            v25 = *(uint32_t *)(i + 77);
            if ( v25 == 2 )
              *(uint32_t *)(i + 3) = 0;
          }
          while ( !v25 );
        }
        v26 = spu_voice_param[0].sample_history[(uint16_t)i[8] + 5 + v32];
        v27 = ((v19 * v26) >> 16) + *((uint32_t *)v23 - 1);
        v28 = ((v22 * v26) >> 16) + *(uint32_t *)v23;
        v29 = *(uint32_t *)(i + 7);
        *((uint32_t *)v23 - 1) = v27;
        result = *(uint32_t *)(i + 11) + v29;
        *(uint32_t *)v23 = v28;
        *(uint32_t *)(i + 7) = result;
        if ( result > 0x1BFFFF )
        {
          result -= 0x1C0000;
          *(uint32_t *)(i + 77) = 0;
          *(uint32_t *)(i + 7) = result;
        }
        v23 += 8;
      }
      while ( (int)v23 < (int)spu_output_samples_end );
    }
    if ( !*(uint32_t *)(i + 3) )
      *(uint32_t *)(i - 1) = 0;
    ++v30;
    v32 += 74;
  }
  return result;
}

static int spucore_set_voiceon(unsigned int a1)
{
  int i;
  int result;

  for ( i = 0; i < 24; ++i )
  {
    if ( (a1 & 1) != 0 )
      result = spucore_voice_key_on(i);
    a1 >>= 1;
  }
  return result;
}

static int spucore_set_pitchmod(unsigned int a1)
{
  int i;
  int result;

  for ( i = 0; i < 24; ++i )
  {
    if ( (a1 & 1) != 0 )
      result = spucore_voice_key_off(i);
    a1 >>= 1;
  }
  return result;
}

static int16_t spucore_write_cnt(int16_t a1)
{
  spu_cnt = a1;
  return a1;
}

static int16_t spucore_write_dma_ctrl(int16_t a1)
{
  spu_dma_ctrl = a1;
  return a1;
}

static int spucore_write_status_hi(int16_t a1)
{
  HIWORD(spu_ctrl_status) = a1 & 0xF800;
  return a1 & 0xF800;
}

static int spucore_set_dma_address(uint16_t a1)
{
  int result;

  result = 8 * a1;
  spu_ram_transfer_addr = result;
  return result;
}

static int spucore_dma_write_fifo(int16_t a1)
{
  int v1;
  int result;

  v1 = spu_ram_transfer_addr;
  *(uint16_t *)((char *)spu_ram + spu_ram_transfer_addr) = a1;
  result = (v1 + 2) & 0x7FFFF;
  spu_ram_transfer_addr = result;
  return result;
}

static int16_t spucore_dma_read_fifo()
{
  int16_t result;

  result = *(uint16_t *)((char *)spu_ram + spu_ram_transfer_addr);
  spu_ram_transfer_addr = (spu_ram_transfer_addr + 2) & 0x7FFFF;
  return result;
}

static int16_t spucore_read_cnt()
{
  return spu_cnt;
}

static int16_t spucore_read_dma_ctrl()
{
  return spu_dma_ctrl;
}

static int16_t spucore_read_status_hi()
{
  return HIWORD(spu_ctrl_status);
}

static int spucore_write_voice_reg(int a1, int a2, uint16_t a3)
{
  int64_t v3;
  int v4;
  int v5;
  int v6;
  double v7;
  int v8;
  int v9;
  uint32_t v10;
  int v11;
  int v12;
  bool v13;
  uint32_t v14;
  int64_t v16;

  LODWORD(v3) = a2;
  switch ( a2 )
  {
    case 0:
      v4 = a1;
      spu_voice_param[v4].volume_left = a3 & 0x3FFF;
      HIDWORD(v3) = (a3 >> 14) & 1;
      spu_voice_param[v4].adsr_lower = HIDWORD(v3);
      spu_voice_param[v4].current_adsr_vol = HIDWORD(v3);
      spu_voice_param[v4].adsr_attack_mode = (a3 >> 13) & 1;
      LODWORD(v3) = a3 & 0x7F;
      spu_voice_param[v4].adsr_attack_step = a3 >> 15;
      spu_voice_param[v4].pitch = v3;
      break;
    case 2:
      v5 = a1;
      spu_voice_param[v5].volume_right = a3 & 0x3FFF;
      HIDWORD(v3) = (a3 >> 14) & 1;
      spu_voice_param[v5].adsr_upper = HIDWORD(v3);
      spu_voice_param[v5].repeat_addr = HIDWORD(v3);
      spu_voice_param[v5].adsr_attack_shift = (a3 >> 13) & 1;
      LODWORD(v3) = a3 & 0x7F;
      spu_voice_param[v5].adsr_decay_shift = a3 >> 15;
      spu_voice_param[v5].start_addr = v3;
      break;
    case 4:
      v6 = a1;
      v16 = a3 & 0x3FFF;
      spu_voice_param[v6].adsr_sustain_mode = v16;
      v7 = (double)v16 * 0.000244140625;
      *(float *)(v6 * 296 + 4609456) = v7;
      v3 = (int64_t)(v7 * 65536.0);
      spu_voice_param[v6].sample_history[0] = v3;
      break;
    case 6:
      spu_voice_param[a1].adsr_sustain_dir = a3;
      LODWORD(v3) = a3;
      break;
    case 8:
      LODWORD(v3) = 296 * a1;
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_sustain_shift + v3) = a3 >> 15;
      HIDWORD(v3) = HIBYTE(a3) & 0x7F;
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_sustain_step + v3) = HIDWORD(v3);
      v8 = (uint8_t)a3 >> 4;
      v9 = a3 & 0xF;
      *(int32_t *)((char *)&spu_voice_param[0].pitch_mod_param + v3) = spu_adsr_attack_rate_table[HIDWORD(v3)];
      HIDWORD(v3) = spu_adsr_decay_rate_table[v8];
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_release_mode + v3) = v9;
      v10 = spu_adsr_sustain_level_table[v9];
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_sustain_level + v3) = v8;
      *(int32_t *)((char *)&spu_voice_param[0].pitch_mod_param2 + v3) = -HIDWORD(v3);
      *(uint32_t *)((char *)&spu_voice_param[0].unknown47 + v3) = v10;
      break;
    case 10:
      LODWORD(v3) = 296 * a1;
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_release_shift + v3) = a3 >> 15;
      HIDWORD(v3) = (a3 >> 14) & 1;
      v11 = (a3 >> 6) & 0x7F;
      v12 = a3 & 0x1F;
      *(uint32_t *)((char *)&spu_voice_param[0].adsr_envelope + v3) = HIDWORD(v3);
      *(uint32_t *)((char *)&spu_voice_param[0].voice_state + v3) = (a3 >> 5) & 1;
      v13 = HIDWORD(v3) == 0;
      HIDWORD(v3) = spu_adsr_sustain_rate_table[v11];
      *(uint32_t *)((char *)&spu_voice_param[0].loop_start_addr + v3) = v11;
      *(uint32_t *)((char *)&spu_voice_param[0].pitch_mod_factor + v3) = v12;
      if ( !v13 )
        HIDWORD(v3) = -HIDWORD(v3);
      v14 = -spu_adsr_release_rate_table[v12];
      spu_voice_param[a1].unknown48 = HIDWORD(v3);
      spu_voice_param[a1].unknown49 = v14;
      break;
    case 12:
      spu_voice_param[a1].unknown17 = a3 << 9;
      LODWORD(v3) = 296 * a1;
      break;
    case 14:
      spu_voice_param[a1].pitch_multiplier = a3;
      break;
    default:
      return v3;
  }
  return v3;
}

static int16_t spucore_read_voice_reg(int a1, int a2)
{
  int v2;
  signed int unknown17;
  uint32_t *p_unknown17;

  switch ( a2 )
  {
    case 4:
      LOWORD(v2) = (int64_t)(*(float *)&spu_voice_param[a1].adpcm_s1 * 4096.0) & 0x3FFF;
      break;
    case 6:
      LOWORD(v2) = spu_voice_param[a1].adsr_sustain_dir;
      break;
    case 12:
      if ( unknown_cd_setting )
      {
        LOWORD(v2) = rand() & 1;
      }
      else
      {
        unknown17 = spu_voice_param[a1].unknown17;
        p_unknown17 = &spu_voice_param[a1].unknown17;
        if ( unknown17 <= 0xFFFFFF )
        {
          if ( unknown17 < 0 )
            *p_unknown17 = 0;
          return (int)*p_unknown17 >> 9;
        }
        else
        {
          *p_unknown17 = 0xFFFFFF;
          return (int)*p_unknown17 >> 9;
        }
      }
      break;
    default:
      LOWORD(v2) = 0;
      break;
  }
  return v2;
}

int spucore_init()
{
  int inited;
  int result;

  dbg_print(" * Init core spu ... ");
  spu_ram_ptr = (int)spu_ram;
  spucore_init_gauss_table();
  inited = spucore_init_dsound();
  if ( forcespu == 1 )
    forcespu = 0;
  spu_xa_samples_left = 0;
  if ( !inited )
  {
    dbg_print("Error: can't open sound handler.\n");
    fatal_error_with_message_box(" * Error can't open sound handler. (try -nosound)\n");
  }
  result = dbg_print(" ok \n");
  spucore_init_flag = 1;
  return result;
}

int spucore_destroy()
{
  int result;

  result = spucore_init_flag;
  if ( spucore_init_flag )
  {
    ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->Stop((LPDIRECTSOUNDBUFFER)pDSBuffer);
    while ( ((LPDIRECTSOUNDBUFFER)pDSBuffer)->lpVtbl->Release((LPDIRECTSOUNDBUFFER)pDSBuffer) )
      ;
    while ( ((LPDIRECTSOUND)ppDS)->lpVtbl->Release((LPDIRECTSOUND)ppDS) )
      ;
    return dbg_print(" * Closing core spu...\n");
  }
  return result;
}

void spucore_dma()
{
  unsigned int v0;
  int v1;
  int v2;
  int *v3;
  int v4;
  int v5;
  unsigned int v6;
  int v7;
  int v8;
  uint16_t fifo;

  v0 = spu_dma_mem_addr;
  v1 = HIWORD(spu_dma_block_size_count);
  v2 = (uint16_t)spu_dma_block_size_count;
  if ( sound_enabled )
  {
    spu_irq_pending_flag = 0;
    if ( *(uint32_t *)spu_dma_chcr_ptr == 0x1000200 )
    {
      v7 = HIWORD(spu_dma_block_size_count) * (uint16_t)spu_dma_block_size_count;
      if ( 2 * v7 )
      {
        v8 = 2 * v7;
        do
        {
          fifo = spucore_dma_read_fifo();
          mem_hw_reg_write_half(v0, fifo);
          v0 += 2;
          --v8;
        }
        while ( v8 );
      }
      if ( dynarec_enabled == 1 && v7 )
        dynarec_invalidate_range(spu_dma_mem_addr, v7);
    }
    else if ( *(uint32_t *)spu_dma_chcr_ptr == 0x1000201 )
    {
      v3 = (int *)mem_dma_read(spu_dma_mem_addr);
      v4 = v1 * v2;
      v5 = spu_ram_transfer_addr;
      v6 = 4 * v4 + spu_ram_transfer_addr;
      if ( v6 >= 0x80000 )
      {
        for ( ; v4; spu_ram_transfer_addr = v5 )
        {
          *(int *)((char *)spu_ram + v5) = *v3;
          v5 = (v5 + 4) & 0x7FFFF;
          ++v3;
          --v4;
        }
      }
      else
      {
        qmemcpy((char *)spu_ram + spu_ram_transfer_addr, v3, 4 * ((unsigned int)(4 * v4) >> 2));
        spu_ram_transfer_addr = v6;
      }
    }
    else if ( (*(uint32_t *)spu_dma_chcr_ptr & 0x1000000) != 0 )
    {
      fatal_error_with_message_box(
        "DMA[4] mode NOT implemented (%08x)\n addr (%08x) num (%04x) size (%04x)\n",
        *(uint32_t *)spu_dma_chcr_ptr,
        spu_dma_mem_addr,
        HIWORD(spu_dma_block_size_count),
        (uint16_t)spu_dma_block_size_count);
    }
  }
}

int16_t spucore_write_register(int16_t a1, int16_t a2)
{
  unsigned int v2;
  int16_t result;

  v2 = (a1 & 0xFFF) - 3072;
  spu_irq_pending_flag = 0;
  if ( v2 < 0x180 )
    return spucore_write_voice_reg(v2 >> 4, a1 & 0xF, a2);
  result = (a1 & 0xFFF) - 3456;
  switch ( a1 & 0xFFF )
  {
    case 0xD80:
      spucore_mainvol_left = a2 & 0x3FFF;
      break;
    case 0xD82:
      LOWORD(spucore_mainvol_right) = a2 & 0x3FFF;
      result = a2 & 0x3FFF;
      break;
    case 0xD84:
      spucore_reverb_vol_left = a2;
      break;
    case 0xD86:
      spucore_reverb_vol_right = a2;
      break;
    case 0xD88:
      result = spucore_set_voiceon((uint16_t)a2);
      break;
    case 0xD8A:
      result = spucore_set_voiceon((uint16_t)a2 << 16);
      break;
    case 0xD8C:
      result = spucore_set_pitchmod((uint16_t)a2);
      break;
    case 0xD8E:
      result = spucore_set_pitchmod((uint16_t)a2 << 16);
      break;
    case 0xD90:
      spucore_pitchmod_enable = (uint16_t)a2 + (spucore_pitchmod_enable & 0xFF0000);
      break;
    case 0xD92:
      result = spucore_pitchmod_enable;
      spucore_pitchmod_enable = (uint16_t)spucore_pitchmod_enable + ((uint16_t)a2 << 16);
      break;
    case 0xD94:
      spucore_noise_mode = (uint16_t)a2 + (spucore_noise_mode & 0xFF0000);
      result = a2;
      break;
    case 0xD96:
      spucore_noise_mode = (uint16_t)spucore_noise_mode + ((uint16_t)a2 << 16);
      break;
    case 0xD98:
      spu_reverb_enable = (uint16_t)a2 + (spu_reverb_enable & 0xFF0000);
      result = a2;
      break;
    case 0xD9A:
      result = spu_reverb_enable;
      spu_reverb_enable = (uint16_t)spu_reverb_enable + ((uint16_t)a2 << 16);
      break;
    case 0xD9C:
      spu_enable = (uint16_t)a2 + (spu_enable & 0xFF0000);
      break;
    case 0xD9E:
      result = spu_enable;
      spu_enable = (uint16_t)spu_enable + ((uint16_t)a2 << 16);
      break;
    case 0xDA2:
      spu_xa_play_data[0x3FFF] = a2;
      break;
    case 0xDA4:
      LOWORD(spu_ctrl_status) = a2;
      result = a2;
      break;
    case 0xDA6:
      result = spucore_set_dma_address(a2);
      break;
    case 0xDA8:
      result = spucore_dma_write_fifo(a2);
      break;
    case 0xDAA:
      result = spucore_write_cnt(a2);
      break;
    case 0xDAC:
      result = spucore_write_dma_ctrl(a2);
      break;
    case 0xDAE:
      result = spucore_write_status_hi(a2);
      break;
    case 0xDB0:
      spu_cd_volume_left = a2;
      result = a2;
      break;
    case 0xDB2:
      spu_cd_volume_right = a2;
      break;
    case 0xDB4:
      spu_ext_volume_left = a2;
      break;
    case 0xDB6:
      spu_ext_volume_right = a2;
      result = a2;
      break;
    default:
      return result;
  }
  return result;
}

int16_t spucore_read_register(int16_t a1)
{
  unsigned int v1;
  unsigned int v2;

  v1 = (a1 & 0xFFF) - 3072;
  spu_irq_pending_flag = 0;
  if ( v1 >= 0x180 )
  {
    switch ( a1 & 0xFFF )
    {
      case 0xD80:
        LOWORD(v2) = spucore_mainvol_left;
        break;
      case 0xD82:
        LOWORD(v2) = spucore_mainvol_right;
        break;
      case 0xD84:
        LOWORD(v2) = spucore_reverb_vol_left;
        break;
      case 0xD86:
        LOWORD(v2) = spucore_reverb_vol_right;
        break;
      case 0xD90:
        LOWORD(v2) = spucore_pitchmod_enable;
        break;
      case 0xD92:
        LOWORD(v2) = HIWORD(spucore_pitchmod_enable);
        break;
      case 0xD94:
        LOWORD(v2) = spucore_noise_mode;
        break;
      case 0xD96:
        LOWORD(v2) = HIWORD(spucore_noise_mode);
        break;
      case 0xD98:
        LOWORD(v2) = spu_reverb_enable;
        break;
      case 0xD9A:
        LOWORD(v2) = HIWORD(spu_reverb_enable);
        break;
      case 0xD9C:
        LOWORD(v2) = spu_enable;
        break;
      case 0xD9E:
        LOWORD(v2) = HIWORD(spu_enable);
        break;
      case 0xDA2:
        LOWORD(v2) = spu_xa_play_data[0x3FFF];
        break;
      case 0xDA4:
        LOWORD(v2) = spu_ctrl_status;
        break;
      case 0xDA6:
        v2 = (unsigned int)spu_ram_transfer_addr >> 3;
        break;
      case 0xDA8:
        LOWORD(v2) = spucore_dma_read_fifo();
        break;
      case 0xDAA:
        LOWORD(v2) = spucore_read_cnt();
        break;
      case 0xDAC:
        LOWORD(v2) = spucore_read_dma_ctrl();
        break;
      case 0xDAE:
        LOWORD(v2) = spucore_read_status_hi();
        break;
      case 0xDB0:
        LOWORD(v2) = spu_cd_volume_left;
        break;
      case 0xDB2:
        LOWORD(v2) = spu_cd_volume_right;
        break;
      case 0xDB4:
        LOWORD(v2) = spu_ext_volume_left;
        break;
      case 0xDB6:
        LOWORD(v2) = spu_ext_volume_right;
        break;
      default:
        LOWORD(v2) = 0;
        break;
    }
  }
  else
  {
    LOWORD(v2) = spucore_read_voice_reg(v1 >> 4, a1 & 0xF);
  }
  return v2;
}

char spucore_play_adpcm(int a1)
{
  char result;
  int v2;

  result = sound_enabled;
  if ( sound_enabled )
  {
    result = sound_use_xa;
    if ( sound_use_xa )
    {
      if ( xa_decode_wrapper(spu_xa_decode_buf_ptr, a1, spu_adpcm_flag) )
      {
        v2 = spu_xa_samples_left;
      }
      else
      {
        v2 = 2016;
        spu_xa_samples_left = 2016;
      }
      result = spu_xa_decode_buf_ptr[0];
      if ( spu_adpcm_flag == 1 )
        spu_xa_playback_rate = *(uint32_t *)spu_xa_decode_buf_ptr;
      spu_adpcm_flag = 0;
      if ( HIWORD(spu_xa_playback_pos) )
      {
        if ( spu_xa_playback_rate < *(int *)spu_xa_decode_buf_ptr )
        {
          result = spu_xa_playback_rate - 6;
          spu_xa_playback_rate += 250;
        }
      }
      else
      {
        if ( 2 * v2 > 0 )
          qmemcpy(spu_xa_play_pcm, spu_xa_decoded_pcm, 4 * ((unsigned int)(2 * v2) >> 1));
        HIWORD(spu_xa_playback_pos) = v2;
        spu_xa_samples_left = 0;
      }
    }
  }
  return result;
}


int spucore_update_thunk()
{
  return spucore_update_dsound();
}

int spucore_freeze(const char *a1, int a2)
{
  int v2;
  char *v3;
  uint32_t *p_adsr_lower;
  int v5;
  int v6;
  int v7;
  int v8;
  int v9;
  int v10;
  int v11;
  int16_t v12;
  int16_t v13;
  int16_t v14;
  uint8_t v15;
  int16_t *v16;
  int v17;
  int v18;
  uint32_t *v19;
  char Buffer[384];
  char v22;

  sprintf(Buffer, "%s", a1);
  *(uint32_t *)&Buffer[3] = 564744;
  gzwrite(a2, (uint8_t *)Buffer, 7u);
  sprintf(Buffer, "ISPU");
  gzwrite(a2, (uint8_t *)Buffer, 8u);
  *(uint64_t *)Buffer = 0x89DF800000002LL;
  gzwrite(a2, (uint8_t *)Buffer, 8u);
  v3 = &Buffer[2];
  p_adsr_lower = &spu_voice_param[0].adsr_lower;
  v5 = 24;
  do
  {
    v6 = 0;
    v7 = 8;
    do
    {
      v8 = v2;
      v2 = 0;
      switch ( v6 )
      {
        case 0:
          v9 = 2 * (uint16_t)(*(uint16_t *)p_adsr_lower | (2 * *((uint16_t *)p_adsr_lower + 12)));
          LOWORD(v9) = *((uint16_t *)p_adsr_lower + 8) | v9;
          v2 = v9 << 13;
          LOWORD(v2) = *((uint16_t *)p_adsr_lower - 8) | v2;
          *((uint16_t *)v3 - 1) = v2;
          break;
        case 2:
          v10 = 2 * (uint16_t)(*((uint16_t *)p_adsr_lower + 2) | (2 * *((uint16_t *)p_adsr_lower + 14)));
          LOWORD(v10) = *((uint16_t *)p_adsr_lower + 10) | v10;
          v2 = v10 << 13;
          LOWORD(v2) = *((uint16_t *)p_adsr_lower - 6) | v2;
          *(uint16_t *)v3 = v2;
          break;
        case 4:
          LOWORD(v2) = *((uint16_t *)p_adsr_lower + 16);
          *((uint16_t *)v3 + 1) = v2;
          break;
        case 6:
          LOWORD(v2) = *((uint16_t *)p_adsr_lower + 18);
          *((uint16_t *)v3 + 2) = v2;
          break;
        case 8:
          v11 = 16 * (uint16_t)(*((uint16_t *)p_adsr_lower + 22) | (*((uint16_t *)p_adsr_lower + 20) << 7));
          LOWORD(v11) = *((uint16_t *)p_adsr_lower + 24) | v11;
          v2 = 16 * v11;
          LOWORD(v2) = *((uint16_t *)p_adsr_lower + 26) | v2;
          *((uint16_t *)v3 + 3) = v2;
          break;
        case 10:
          LOBYTE(v2) = *((uint8_t *)p_adsr_lower + 60) | (2 * *((uint8_t *)p_adsr_lower + 56));
          LOBYTE(v12) = 0;
          HIBYTE(v12) = v2;
          *((uint16_t *)v3 + 4) = *((uint16_t *)p_adsr_lower + 36)
                             | (32 * (*((uint16_t *)p_adsr_lower + 34) | (2 * (*((uint16_t *)p_adsr_lower + 32) | v12))));
          break;
        case 12:
          v2 = (int)p_adsr_lower[19] >> 9;
          *((uint16_t *)v3 + 5) = v2;
          break;
        case 14:
          LOWORD(v2) = *((uint16_t *)p_adsr_lower + 40);
          *((uint16_t *)v3 + 6) = v2;
          break;
        default:
          v2 = v8;
          break;
      }
      v6 += 2;
      --v7;
    }
    while ( v7 );
    p_adsr_lower += 74;
    v3 += 16;
    --v5;
  }
  while ( v5 );
  v13 = spucore_mainvol_left;
  v14 = spucore_mainvol_right;
  v15 = BYTE2(spucore_pitchmod_enable);
  v16 = (int16_t *)&v22;
  v17 = 0;
  v18 = 64;
  do
  {
    v19 = p_adsr_lower;
    p_adsr_lower = nullptr;
    switch ( v17 )
    {
      case 0:
        *v16 = v13;
        break;
      case 2:
        *v16 = v14;
        break;
      case 4:
        LOWORD(p_adsr_lower) = spucore_reverb_vol_left;
        goto LABEL_37;
      case 6:
        LOWORD(p_adsr_lower) = spucore_reverb_vol_right;
        goto LABEL_37;
      case 16:
        LOWORD(p_adsr_lower) = spucore_pitchmod_enable;
        goto LABEL_37;
      case 18:
        p_adsr_lower = (uint32_t *)v15;
        goto LABEL_37;
      case 20:
        LOWORD(p_adsr_lower) = spucore_noise_mode;
        goto LABEL_37;
      case 22:
        p_adsr_lower = (uint32_t *)BYTE2(spucore_noise_mode);
        goto LABEL_37;
      case 24:
        LOWORD(p_adsr_lower) = spu_reverb_enable;
        goto LABEL_37;
      case 26:
        p_adsr_lower = (uint32_t *)BYTE2(spu_reverb_enable);
        goto LABEL_37;
      case 28:
        LOWORD(p_adsr_lower) = spu_enable;
        goto LABEL_37;
      case 30:
        p_adsr_lower = (uint32_t *)BYTE2(spu_enable);
        goto LABEL_37;
      case 34:
        LOWORD(p_adsr_lower) = spu_xa_play_data[0x3FFF];
        goto LABEL_37;
      case 36:
        LOWORD(p_adsr_lower) = spu_ctrl_status;
        goto LABEL_37;
      case 42:
        LOWORD(p_adsr_lower) = spucore_read_cnt();
        goto LABEL_37;
      case 44:
        LOWORD(p_adsr_lower) = spucore_read_dma_ctrl();
        goto LABEL_37;
      case 46:
        LOWORD(p_adsr_lower) = spucore_read_status_hi();
        goto LABEL_37;
      case 48:
        LOWORD(p_adsr_lower) = spu_cd_volume_left;
        goto LABEL_37;
      case 50:
        LOWORD(p_adsr_lower) = spu_cd_volume_right;
        goto LABEL_37;
      case 52:
        LOWORD(p_adsr_lower) = spu_ext_volume_left;
        goto LABEL_37;
      case 54:
        LOWORD(p_adsr_lower) = spu_ext_volume_right;
LABEL_37:
        *v16 = (int16_t)p_adsr_lower;
        break;
      default:
        p_adsr_lower = v19;
        break;
    }
    v17 += 2;
    ++v16;
    --v18;
  }
  while ( v18 );
  gzwrite(a2, (uint8_t *)Buffer, 0x200u);
  gzwrite(a2, (uint8_t *)spu_ram, 0x80000u);
  gzwrite(a2, (uint8_t *)spu_xa_decode_buf_ptr, 0x8020u);
  return gzwrite(a2, (uint8_t *)spu_voice_param, 0x1BC0u);
}

int spucore_unfreeze(int a1, uint32_t *a2)
{
  char *v2;
  uint32_t *p_adsr_lower;
  unsigned int v4;
  uint16_t v5;
  int v6;
  int v7;
  uint16_t v8;
  int v9;
  double v10;
  double v11;
  uint16_t v12;
  char v13;
  int v14;
  int v15;
  int v16;
  int v17;
  int v18;
  int v19;
  uint16_t v20;
  int v21;
  int v22;
  int v23;
  int v24;
  int v25;
  int v26;
  int v27;
  int v28;
  int v29;
  int v30;
  int16_t *v31;
  int v32;
  int result;
  int v34;
  int v35;
  int v36;
  int v37;
  char Str1[12];
  char v39[384];
  char v40;

  gzread(a2, v39, 7);
  gzread(a2, Str1, 8);
  gzread(a2, v39, 8);
  gzread(a2, v39, 512);
  v2 = v39;
  p_adsr_lower = &spu_voice_param[0].adsr_lower;
  v34 = 24;
  do
  {
    v4 = 0;
    v36 = 8;
    do
    {
      if ( v4 <= 0xE )
      {
        switch ( v4 )
        {
          case 0u:
            v5 = *(uint16_t *)v2;
            v6 = (*(uint16_t *)v2 >> 14) & 1;
            v7 = *(uint16_t *)v2 & 0x3FFF;
            *p_adsr_lower = v6;
            p_adsr_lower[2] = v6;
            *(p_adsr_lower - 4) = v7;
            p_adsr_lower[4] = (v5 >> 13) & 1;
            p_adsr_lower[6] = v5 >> 15;
            *(p_adsr_lower - 2) = v5 & 0x7F;
            break;
          case 1u:
          case 3u:
          case 5u:
          case 7u:
          case 9u:
          case 0xBu:
          case 0xDu:
            break;
          case 2u:
            v8 = *((uint16_t *)v2 + 1);
            *(p_adsr_lower - 3) = v8 & 0x3FFF;
            v9 = (v8 >> 14) & 1;
            p_adsr_lower[1] = v9;
            p_adsr_lower[3] = v9;
            p_adsr_lower[5] = (v8 >> 13) & 1;
            p_adsr_lower[7] = v8 >> 15;
            *(p_adsr_lower - 1) = v8 & 0x7F;
            break;
          case 4u:
            v10 = (double)(*((uint16_t *)v2 + 2) & 0x3FFF);
            p_adsr_lower[8] = *((uint16_t *)v2 + 2) & 0x3FFF;
            v11 = v10 * 0.000244140625;
            *((float *)p_adsr_lower + 24) = v11;
            p_adsr_lower[25] = (int64_t)(v11 * 65536.0);
            break;
          case 6u:
            p_adsr_lower[9] = *((uint16_t *)v2 + 3);
            break;
          case 8u:
            v12 = *((uint16_t *)v2 + 4);
            v13 = v2[9];
            p_adsr_lower[10] = v12 >> 15;
            v14 = v13 & 0x7F;
            v15 = (uint8_t)v12 >> 4;
            p_adsr_lower[11] = v14;
            v16 = spu_adsr_attack_rate_table[v14];
            p_adsr_lower[12] = v15;
            v17 = spu_adsr_decay_rate_table[v15];
            v18 = v12 & 0xF;
            p_adsr_lower[65] = v16;
            v19 = spu_adsr_sustain_level_table[v18];
            p_adsr_lower[13] = v18;
            p_adsr_lower[66] = -v17;
            p_adsr_lower[67] = v19;
            break;
          case 0xAu:
            v20 = *((uint16_t *)v2 + 5);
            p_adsr_lower[14] = v20 >> 15;
            v21 = (v20 >> 14) & 1;
            v22 = (v20 >> 6) & 0x7F;
            v23 = (v20 >> 5) & 1;
            v24 = v20 & 0x1F;
            p_adsr_lower[15] = v21;
            p_adsr_lower[16] = v22;
            p_adsr_lower[17] = v23;
            p_adsr_lower[18] = v24;
            if ( v21 )
            {
              v26 = -spu_adsr_sustain_rate_table[v22];
              v25 = -spu_adsr_release_rate_table[v24];
              p_adsr_lower[68] = v26;
            }
            else
            {
              p_adsr_lower[68] = spu_adsr_sustain_rate_table[v22];
              v25 = -spu_adsr_release_rate_table[v24];
            }
            p_adsr_lower[69] = v25;
            break;
          case 0xCu:
            v37 = *((uint16_t *)v2 + 6) << 9;
            p_adsr_lower[19] = v37;
            *((float *)p_adsr_lower + 26) = (double)v37 * 0.000030517578125;
            break;
          case 0xEu:
            p_adsr_lower[20] = *((uint16_t *)v2 + 7);
            break;
        }
      }
      v4 += 2;
      --v36;
    }
    while ( v36 );
    v2 += 16;
    p_adsr_lower += 74;
    --v34;
  }
  while ( v34 );
  v27 = spucore_pitchmod_enable;
  v28 = spucore_noise_mode;
  v29 = spu_reverb_enable;
  v30 = spu_enable;
  v31 = (int16_t *)&v40;
  v32 = 0;
  v35 = 64;
  do
  {
    switch ( v32 )
    {
      case 0:
        spucore_mainvol_left = *v31;
        break;
      case 2:
        LOWORD(spucore_mainvol_right) = *v31;
        break;
      case 4:
        spucore_reverb_vol_left = *v31;
        break;
      case 6:
        spucore_reverb_vol_right = *v31;
        break;
      case 16:
        v27 = (uint16_t)*v31;
        break;
      case 18:
        v27 = ((uint16_t)*v31 << 16) + (uint16_t)v27;
        break;
      case 20:
        v28 = (uint16_t)*v31;
        break;
      case 22:
        v28 = ((uint16_t)*v31 << 16) + (uint16_t)v28;
        break;
      case 24:
        v29 = (uint16_t)*v31;
        break;
      case 26:
        v29 = ((uint16_t)*v31 << 16) + (uint16_t)v29;
        break;
      case 28:
        v30 = (uint16_t)*v31;
        break;
      case 30:
        v30 = ((uint16_t)*v31 << 16) + (uint16_t)v30;
        break;
      case 34:
        spu_xa_play_data[0x3FFF] = *v31;
        break;
      case 36:
        LOWORD(spu_ctrl_status) = *v31;
        break;
      case 42:
        spucore_write_cnt(*v31);
        break;
      case 44:
        spucore_write_dma_ctrl(*v31);
        break;
      case 46:
        spucore_write_status_hi(*v31);
        break;
      case 48:
        spu_cd_volume_left = *v31;
        break;
      case 50:
        spu_cd_volume_right = *v31;
        break;
      case 52:
        spu_ext_volume_left = *v31;
        break;
      case 54:
        spu_ext_volume_right = *v31;
        break;
      default:
        break;
    }
    v32 += 2;
    ++v31;
    --v35;
  }
  while ( v35 );
  spu_reverb_enable = v29;
  spu_enable = v30;
  spucore_noise_mode = v28;
  spucore_pitchmod_enable = v27;
  gzread(a2, (char *)spu_ram, 0x80000);
  gzread(a2, spu_xa_decode_buf_ptr, 32800);
  spu_xa_playback_rate = *(uint32_t *)spu_xa_decode_buf_ptr;
  if ( spu_xa_samples_left > 4096 )
    spu_xa_samples_left = 0;
  result = strncmp(Str1, "ISPU", 4u);
  if ( !result )
    return gzread(a2, (char *)spu_voice_param, 7104);
  return result;
}


/* Decompiled globals (previously generated in src/_gen) */
unsigned int audio_buffer_delay = 0x1;
unsigned int audio_buffer_size_bytes = 0x4800;
unsigned int audio_sample_rate_mode = 0x3;
unsigned int audio_stereo_flag = 0x1;
unsigned char spu_gauss_kernel[10] = {0x1b, 0xe8, 0xea, 0x8a, 0xbb, 0xf6, 0xb0, 0x51, 0x19, 0xf1};
unsigned char spu_gauss_table;
unsigned char spu_xa_decoded_pcm[0x8004];
unsigned char spu_dsound_write_cursor;
unsigned char spu_xa_play_data[0x8000];
unsigned char spu_output_samples[0x200];
unsigned char spu_dsound_wave_format_ptr[0x14];
unsigned int spu_adsr_attack_rate_table[9] = {0x1c3c00, 0x1c3c00, 0x1c3c00, 0x1c3c00, 0x1c3c00, 0x17be00, 0x13f6f8, 0x10c9cb, 0xe1e00};
unsigned int spu_adsr_decay_rate_table[6] = {0x31b343, 0x1ddace, 0x11ef00, 0xac5cb, 0x67898, 0x3e31b};
unsigned int spu_adsr_sustain_rate_table[6] = {0x5f8e2c, 0x505a27, 0x43915e, 0x38d14b, 0x2fc716, 0x282d13};
unsigned int spu_adsr_sustain_level_table[6] = {0xfffff, 0x1fffff, 0x2fffff, 0x3fffff, 0x4fffff, 0x5fffff};
unsigned int spu_adsr_release_rate_table[8] = {0x6666, 0x6666, 0x3333, 0x2222, 0x1999, 0x147a, 0x1111, 0xea0};
unsigned int spu_xa_playback_rate = 0x1;
unsigned int spu_adpcm_coeffs_a[1] = {0x0};
unsigned int spu_adpcm_coeffs_b[9] = {0x0, 0x3c, 0x0, 0x73, 0xffffffcc, 0x62, 0xffffffc9, 0x7a, 0xffffffc4};
unsigned int spu_ctrl_status;
unsigned int spu_stereo_flag;
unsigned int spu_dsound_play_cursor;
unsigned int spu_reverb_enable;
unsigned int spu_xa_decode_buf_ptr[1];
unsigned int spu_xa_stereo_flag;
unsigned int spu_xa_samples_left;
unsigned int spu_enable;
unsigned int spu_sample_rate_mode;
unsigned int spu_output_samples_end[0x7f];
unsigned int spu_ram_transfer_addr;
unsigned int spu_dsound_desc_size;
unsigned int spu_dsound_desc_flags;
unsigned int spu_dsound_desc_buffer_bytes;
unsigned int spu_mute_flag;
unsigned int spu_dsound_buffer_pos;
unsigned int spu_irq_pending_flag;
unsigned int spu_irq_counter;
unsigned int spu_xa_playback_pos;
unsigned int spu_dma_mem_addr;
unsigned int spu_dma_block_size_count;
unsigned int flt_4F75AC;
unsigned int lock_ptr1;
unsigned int lock_ptr2;
unsigned int lock_size1;
unsigned int lock_size2;
unsigned int pDSBuffer = 0x0;
unsigned int ppDS;
unsigned char spu_ram[0x80000];
unsigned int spu_ram_ptr;
SPU_VOICE_PARAM spu_voice_param[24];
unsigned int spucore_init_flag;
unsigned short spucore_mainvol_left;
unsigned int spucore_mainvol_right;
unsigned int spucore_noise_mode;
unsigned int spucore_output_buffer;
unsigned int spucore_pitchmod_enable;
unsigned short spucore_reverb_vol_left;
unsigned short spucore_reverb_vol_right;
unsigned char temp_samples[0x1c20];
unsigned short spu_dma_ctrl;
unsigned short spu_cd_volume_left;
unsigned short spu_cd_volume_right;
unsigned short spu_xa_play_pcm[1];
unsigned short spu_cnt;
unsigned short spu_ext_volume_left;
unsigned short spu_ext_volume_right;
unsigned short spu_adpcm_error_count;
